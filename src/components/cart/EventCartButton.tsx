...
export const EventCartButton: React.FC<EventCartButtonProps> = ({ variant = "floating" }) => {
  ...
  if (variant === "inline") {
    return (
      <button
        onClick={openCart}
        className="relative inline-flex items-center gap-2 px-4 py-2 rounded-full bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs sm:text-sm transition-all border border-rose-200 shadow-xs cursor-pointer active:scale-95"
      >
        <ShoppingBag className="w-4 h-4 text-rose-500" />
        <span>Carrinho de Evento</span>
        {totalItemsCount > 0 && (
  ...
  return (
    <button
      onClick={openCart}
      aria-label="Abrir Carrinho de Evento"
    ...
    >
      ...
      <div className="hidden sm:flex flex-col text-left">
        <span className="font-bold text-xs text-white">Meu Carrinho</span>
